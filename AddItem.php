...
mysql_close($connenction_link);
  ...
  header("Location: Home(Teacher).html?msg=3");

}
else{

  echo "   <h2> هذه المادة موجودة من قبل </h2>";
}

}
 
?>      
             	<form class="" method="post" action="AddItem.php">
        	   	<input type="text"  name="CourseName" required placeholder="إسم المادة" />
				<br>
        		<input type="text"  name="Category" required placeholder="تصنيف المادة" />
				<br>
        		<input type="text"  name="Picture" required placeholder="صورة للمادة" />
				<br>
        		<input type="submit" name="submit" value="أضف المادة" />
